...
export async function initializeParser(context: vscode.ExtensionContext): Promise<void> {
    const fs = await import("node:fs/promises");

    const wasmPath = context.asAbsolutePath(path.join("parsers", "tree-sitter-compact.wasm"));

    // Load tree-sitter WASM binary and pass it directly
    const treeSitterWasmPath = context.asAbsolutePath(path.join("parsers", "web-tree-sitter.wasm"));
